...
class QuestModalReg extends Component {
  render() {
    ...
    const redirectToAuth = !this.props.loggedIn ? (
      <Redirect
        from={this.props.url}
        to={'/auth/' + encodeURIComponent(this.props.url)}
      />
    ) : (
      <React.Fragment />
    )
    ...
    const onFinish = (values) => {
      this.props.handleTeamCreation(values.teamname, this.props.quest_id)
    }
    ...
    return (
      <div>
        ...
        >
          <p>
            <Title level={3}>Регистрация на квест</Title>
          </p>
          {errorMessage}
          <Form
            name='quest-reg'
          ...
          >
            <p>
              ...
              После регистрации станет доступна ссылка для приглашения других
              участников команды
            </p>
            <p>
              <Form.Item>
...
const mapDispatchToProps = (dispatch) => ({
  ...
  handleTeamCreation: (teamname, questId) =>
    dispatch(Api.QuestRegistration.handleTeamCreation(teamname, questId)),
})
