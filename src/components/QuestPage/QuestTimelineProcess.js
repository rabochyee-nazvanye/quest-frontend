...
function QuestTimelineProcess(props) {
  ...
  const componentsHaveCommand = {
    ...
    resultsavailable: (
      <Button
        ...
        onClick={function () {
          document.location =
            'https://docs.google.com/forms/d/e/1FAIpQLSeDcmG1Iai2nmFUDMR1pXMKMY6l5Tp5iVyDO_H8sSvMOvHhhA/viewform'
        }}
      >
        <StarOutlined />
        Оставить отзыв
  ...
  if (props.registered === true) buttons = componentsHaveCommand

  const remainingTime =
    new Date(props.quest.endDate).getHours() * 60 +
    new Date(props.quest.endDate).getMinutes() -
  ...
  const regDeadline =
    props.quest.registrationDeadline === new Date('1970-01-01').toISOString()
      ? 'Открыта до конца квеста'
      : 'до ' +
        new Intl.DateTimeFormat('default', commonOptions).format(
          new Date(props.quest.registrationDeadline)
        )
  const startTime = new Intl.DateTimeFormat('default', commonOptions)
    .format(new Date(props.quest.startDate))
    .toString()
  const endTime = new Intl.DateTimeFormat('default', commonOptions)
    .format(new Date(props.quest.endDate))
    .toString()

  return (
    <QuestTimelineTemplate
