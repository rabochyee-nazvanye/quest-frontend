...
class QuestPage extends Component {
  ...
  getRepresentationByState() {
    ...
    if (this.props.questIsFetching) return <Spin />
    else {
      if (this.props.quest === null) return <Spin />
      else {
        if (
          !this.props.quest.isInfinite &&
          this.props.quest.status !== 'registrastionover' &&
          this.props.quest.status !== 'scheduled'
        )
          questBottom = (
            <div>
              <QuestDescriptionLogic quest={this.props.quest} />
              {/* TODO: отображать компонент ниже только после того, как открыли результаты в лидерборде [РЕДИЗАЙН ЕГОРА] */}
              <QuestResultsLogic id={this.props.quest.id} />
            </div>
          )
        else questBottom = <QuestDescriptionLogic quest={this.props.quest} />

        if (!this.props.quest.isInfinite) {
          timing = (
            <QuestTimelineProcess
        ...
        if (this.props.quest?.timeToCompleteInMinutes === 0) {
          ...
          questDeadlineAlert = (
            <QuestWithDeadlineAlert
              timeToCompleteInMinutes={this.props.quest.timeToCompleteInMinutes}
            />
          )

          if (this.props.questStatus?.tasksRead) {
            questDeadlineAlert = (
              <QuestDeadlineAlert deadline={this.props.questStatus.deadline} />
            )
          }
        }
        return (
          <section className={'quest-page'}>
            <QuestMinimalInfo quest={this.props.quest} user={this.props.user} />
            <QuestCreatorControls
              quest={this.props.quest}
