...
function TooltipBotTemplate () {
  return (
    <span>Нужен для авторизации в боте: <a href={'https://tglink.ru/questspacebot'}>@questspacebot</a></span>
  )
}
    ...
    <React.Fragment>
      ...
      <h2 className={'title__subtitle title__blue title__close '} >Команда: {props.team.name} <span className={'title__black title__low-opacity'}> | </span>
        <Tooltip title={<TooltipBotTemplate/>}>
          <span className={'title__underscored'}>{props.team.inviteCode}</span>
        </Tooltip>
      </h2>
    </React.Fragment>
  )
