...
export default class AuthApi {
    ...
    return (dispatch) => {
      ...
      dispatch(
        this.fetchUserToken(
          ...
          password,
          true,
          this.config.BACKEND_AUTH_REGISTER_PATH
        )
      )
    }
  }
  ...
  getUserByToken(token) {
    return (dispatch) => {
      ...
      return getWithToken(
        this.config.BASE_URL + this.config.BACKEND_AUTH_FETCH_PATH
      ).then((response) => {
        if (response.ok) {
          response.json().then((json) => {
            const payload = {
  ...
  oAuth(props) {
    ...
    return (dispatch) => {
      ...
      return fetch(this.config.BASE_URL + '/externalauth', options).then(
        (response) => {
          if (response.ok) {
            response.json().then((json) => {
              const token = json.token.result
