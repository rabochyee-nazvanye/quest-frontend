...
import {
  ...
  receiveQuestTasks,
  requestQuestTasks,
  sendQuestTaskAttempt,
  sendQuestTaskHintRequest,
  updateQuestTasks,
...
export default class QuestPlaymodeApi {
  ...
  sendTaskAttempt(taskId, attemptText) {
    return (dispatch) => {
      ...
      this.commonApi
        .postWithToken(`${this.config.BASE_URL}/tasks/${taskId}/attempts`, {
  ...
  getTaskHint(taskId, hintNumber) {
    return (dispatch) => {
      ...
      this.commonApi
        .postWithToken(
          `${this.config.BASE_URL}/tasks/${taskId}/hintrequests/${hintNumber}`,
          {}
        )
        .then((response) => {
          if (response.ok) {
  ...
  getQuestTasks(questId) {
    return (dispatch) => {
      ...
      this.commonApi
        .getWithToken(`${this.config.BASE_URL}/quests/${questId}/tasks`)
        .then((response) => {
          if (response.ok) {
            response.json().then((json) => {
